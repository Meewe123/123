...
orbits in. Slip through the gap in each incoming ring. The first launch shows
a three-line tutorial.

Notes for review:
- The app makes no network requests. It works fully offline, including in
  airplane mode.
- There are no ads, no in-app purchases and no third-party SDKs.
...
- [ ] `npm run verify` passes (icons regenerate, 39 unit tests, 37 e2e checks).
...
| 3.1.1 In-App Purchase | Selling outside IAP | Nothing is sold |
