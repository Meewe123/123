# App Store listing — Orbital Rush

Everything below is the copy to paste into App Store Connect. Character limits
are Apple's; the counts in brackets are what the current text uses.
...
4+ — no objectionable content, no ads, no user-generated content, no web views
pointing at third-party content, no gambling, no in-app purchases.
...
`https://orbitalrush.example.com/support` — replace with your live URL before
submitting. A single page listing an email address satisfies Apple.

## Marketing URL (optional)

`https://orbitalrush.example.com`

## Privacy Policy URL (required)

`https://orbitalrush.example.com/privacy` — publish `store/privacy-policy.md`
at this address. See `store/app-privacy.md` for the questionnaire answers.
...
`npm run screenshots` renders the required sets at the exact pixel sizes App
Store Connect accepts:

| Device                          | Size (px)   | Required |
| ------------------------------- | ----------- | -------- |
| 6.9" (iPhone 17 Pro Max class)  | 1320 × 2868 | yes      |
| 6.5" (iPhone 11 Pro Max class)  | 1242 × 2688 | yes      |
| 13" iPad Pro                    | 2064 × 2752 | if iPad is a supported device |
...
15–30 s, portrait, no voiceover. Record a real run: open on the title screen,
start a run, land three PERFECTs, take a power-up, die on a Storm ring. The
loop sells itself; do not cut away from gameplay.
