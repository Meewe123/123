...
| **The tests** | `tests/` — 39 head-less simulation/meta tests plus a 37-check browser end-to-end run. |
...
Because of that, the generator can place each gap **inside the arc the player
can still reach** from the previous gap:

```
budget = separation × (player angular speed ÷ ring speed) × safety
```

There is no such thing as an unwinnable ring. Difficulty comes from narrower
gaps, faster spin, more gaps, and tighter timing — never from geometry you
could not have solved. `tests/world.test.mjs` asserts this invariant directly,
...
Collisions are resolved by solving for the exact moment the ring's radius met
your orbit and rewinding both bodies to that instant, so the verdict is
identical at 60 Hz, 120 Hz, or in a head-less test.
...
| 25 | 0.74 s | Two-gap rings appear |
| 80 | 0.54 s | Reversing and twin rings |
| 200 | 0.41 s | Narrow gaps, drift, ghost rings |
| 350+ | 0.36 s | Everything at once |
...
```bash
npm test          # 39 head-less tests: simulation, fairness, economy, assets
npm run test:e2e  # 37 checks in real Chromium at phone/tablet/landscape sizes
npm run verify    # icons + both suites
```
...
* `TUNE` — orbit radius, ring speed, spacing, forgiveness, chain rules, revive
  price.
